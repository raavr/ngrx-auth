...
import { LoadAccountsSuccess } from '../actions/accounts.action';
...
describe("Accounts Reducer", () => {
  ...
  it('should add many users to the store', () => {
    ...
    const action = new LoadAccountsSuccess([user3, user4]);
    const expResult = {
      ids: [
        ...initialState.ids,
        user3.id,
  ...
  it('should add only new users if users already exist', () => {
    const user3 = { id: '3', name: 'Test', email: 'test@example.com' }
    const action = new LoadAccountsSuccess([user2, user3]);
    const expResult = {
      ids: [
        ...initialState.ids,
        user3.id,
